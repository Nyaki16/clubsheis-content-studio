    ...
    <div className="space-y-3">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${
          isDragActive
            ...
            : 'border-border hover:border-brown/40 bg-cream-dark/40'
        }`}
      >
        <input {...getInputProps()} />
        <svg
        ...
        <p className="font-ui text-sm text-text-secondary">{label}</p>
      ...
      {files.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {files.map((file, i) => (
            <div
              key={i}
              className="flex items-center gap-2 px-3 py-1.5 bg-cream-dark rounded-lg text-sm font-ui"
            >
      ...
      <div>
        ...
        <input
          ...
          placeholder="https://..."
          className="w-full mt-1 px-4 py-2.5 border border-border-light rounded-xl font-ui text-sm focus:outline-none focus:border-brown focus:ring-1 focus:ring-brown/20 bg-white"
        />
      </div>
